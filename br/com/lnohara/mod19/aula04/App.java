...
import Modulo_14.console.dao.ClienteMapDAO;
import Modulo_14.console.dao.IClienteDAO;
...
public class App {
    ...
    private static void cadastrar(String dados) {
        String[] seis = {"nome","cpf", "0", "end", "0", "cidade", "estado"};
        String[] dadosSeparados = dados.split(",");

        if(dadosSeparados.length <= 1){
            JOptionPane.showMessageDialog(null, "Nome e CPF são obrigatórios ", "Sucesso",JOptionPane.INFORMATION_MESSAGE);
            return;
        }

        for (int i = 0; i < dadosSeparados.length; i++){
            seis[i] = dadosSeparados[i];
        }

        Cliente cliente = new Cliente(seis[0],seis[1],seis[2],seis[3],seis[4],seis[5],seis[6]);
